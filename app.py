...
authenticator = Authenticate(
    ...
    cookie_name='zomato_roster_cookie',
    cookie_key='secure_random_string_123',
...
if st.session_state.get('connected'):
    ...
    if not user_email.endswith('@zomato.com'):
        st.error(f"Access Denied: {user_email} is not an authorized Zomato account.")
        st.stop() 
        ...
        if st.button("🚀 Generate Roster", use_container_width=True):
            with st.spinner("Crunching the numbers and balancing shifts... Please wait."):
                try:
                    ...
                    # ==========================================
                    # 5. ROSTER GENERATION LOGIC (V10 Engine)
                    # ==========================================
                            ...
                            if not is_zero_wo and len(assigned_wos) < dynamic_wo_target:
                                ...
                                for e in rem_active:
                                    ...
                                    if state['WOs_Remaining'] > 0:
                                        ...
                                        # If using a WO today leaves them with insufficient WOs to cover the rest of the month's 9-day rules, ban it.
    ...
    st.info("Please log in with your Zomato Google account to access the app.")
